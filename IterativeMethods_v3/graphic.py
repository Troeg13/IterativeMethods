...
#точное решение
Data_u = pd.read_csv("table_u.csv", sep = ',', header=None, index_col=None, encoding='cp1251')
...
ax_2.set_xlabel('x')
ax_2.set_ylabel('y')
ax_2.set_zlabel('U*(x,y)')
ax_2.set_title('График точного решения U*(x,y)')
...
ax_v0.set_xlabel('x')
ax_v0.set_ylabel('y')
ax_v0.set_zlabel('V(x,y)')
ax_v0.set_title('График начального приближения V0(x, y) ')

#разность точного и численного
Data_z = pd.read_csv("table_z.csv", sep = ',', header=None, index_col=None, encoding='cp1251')
...
fig_z = plt.figure(figsize = (8, 6)) #создаём трёхмерную систему координат
...
ax_z.set_xlabel('x')
ax_z.set_ylabel('y')
ax_z.set_zlabel('U*(x,y) - V(x,y)', labelpad = 15)
ax_z.set_title('График U*(x,y) - V(x,y) где U* - точное решение, V - численное решение')
plt.show()